...
`Project_Atrium_2026-05-02.md` (Sessions 32–34 delta), both of which
remain in the repo as historical record.
...
> require full recursive test output, not summary numbers.
...
`initAnimations()` completes, the renderer runs
...
The renderer registers `mixer.addEventListener('finished', ...)` so that
when a `LoopOnce` action finishes naturally, `anim.stop()` is called on
...
## `packages/renderer-three/` (Session 34)

Three.js-specific glue. New as of Session 34, with the bridge as its
first tenant. Long-term home for all Three.js coupling — AnimationMixer
integration code currently duplicated between `apps/client` and
`tools/som-inspector` is a candidate for future extraction, but not
urgent.
...
19 unit tests as of Session 34.

---

## Client Package (`packages/client`)

Five classes (four pre-32 + pointer dispatch added to AtriumClient), zero
Three.js or DOM dependency — portable across browser UI, headless tests,
and future bot clients.
...
### AvatarController, NavigationController, AnimationController

Unchanged from April 17. Lifecycle, navigation modes, and animation
semantics as previously documented. See `Project_Atrium_2026-04-17.md`
for full per-class detail if needed.
...
client stack with an inspection-focused UI. Shares animation
integration patterns with `apps/client`.
...
Fork of `space.gltf` with two loopable animations (`CrateRotate`,
`CrateBob`). Both seamlessly loopable. No `extras.atrium.playback`
authored (start stopped).
...
| `apps/client` — Three.js animation integration | ✅ Complete (Sessions 28–31) |
...
- **Pointer event bubbling not implemented** — leaf-only dispatch.
  Lamp parent in `space.gltf` cannot receive clicks because it has no
  mesh of its own. First concrete need surfaced in Session 34
  playground smoke testing.
...
- **Load while connected** — Loading a new `.gltf` while connected does
  not disconnect. SOM mismatch. (Known, deferred — but useful as a
  diagnostic artifact during connect-time bug investigation, see April
  17 doc.)
...
- **Test count regressions or misreports.** Cross-check the build
  ...
  trusting them. Future briefs should require full recursive output.
- **Smoke test plans rewritten rather than amended.** Session 32 and
  33 both saw tests dropped during regeneration. Future briefs should
  say "preserve existing tests, add to them, don't replace."
...
- **Bug-for-bug compatibility on migration.** When extracting a shared
  ...
  that the abstraction's shape is wrong.
...
- **Lifecycle arcs take more sessions than briefs predict.** The
  ...
  initial brief implied. Each session exposed a connect-time
  lifecycle assumption that the previous design hadn't modeled. When a
  future feature touches the connect-time path, plan for two or three
  iterations rather than one. Bubbling is likely to have a similar
  shape — design first, then expect implementation to surface
  questions the design didn't fully answer.
...
- `Project_Atrium_2026-04-17.md` — Sessions 1–31 reference (animation
  arc, pre-pointer architecture)
- `Project_Atrium_2026-05-02.md` — Sessions 32–34 delta + reflective
  notes (superseded by this document)
