# Project Atrium
## 2026-06-14 · As of Session 41

This document is the canonical project handoff. It supersedes
`Project_Atrium_2026-05-14.md` (Sessions 1–37) and folds in the full
content of `DESIGN-Rendering-Independence.md` and `DESIGN-SOMLight-Naming.md`
so this document is self-contained — the next session does not need any
predecessor. All three are retained in the repo as historical record.
...
- The `node` field in `set` messages targets any named SOM object — nodes,
  animations, lights, or `__document__` — resolved via `som.getObjectByName()`
  (Session 27). The field accepts any non-empty string; dotted names like
  `"Sun.light"` are valid and route correctly.
...
| `SOMCamera` | glTF-Transform `Camera` | `type`, `yfov`, `znear`, `zfar`, `aspectRatio`, `xmag`, `ymag` — **read-only in current implementation; mutability + events + registration are next** |
...
| `SOMCamera` | **NO** — next session work |
...
Consistent with existing `.mesh` and `.camera` accessors.
...
**`SOMCamera` completeness: next implementation session.** See below.
...
`AnimationBridge`. **Not yet implemented — follow-on session after `SOMCamera`.**
...
Because `SOMLight` is mutable networked state, a world can start with no
scene-graph lights (Tier A/C) and have a peer add a `SOMLight` at runtime.
The recommended behavior: Tier C fallback is suppressed whenever ≥1
`SOMLight` exists; predicate re-evaluated on `som:add`/`som:remove`;
renderer walks current SOM state when ready (Principle 11), mirroring
`replayPlayingAnimations`. **This reconcile behavior is implemented as part
of the `Stage` session, not independently.**

---

## `SOMCamera` Completeness — Design (not yet implemented)

`SOMCamera` already exists as a thin wrapper (`type`, `yfov`, `znear`,
`zfar`, `aspectRatio`, `xmag`, `ymag`) but its properties are currently
read-only and it is **not registered in `_objectsByName`**. The gap:

1. **Mutability + events.** Every property needs a setter that fires
   `mutation`, so a peer editing fov broadcasts and reconciles.
2. **Registration.** `SOMCamera` must be registered in `_objectsByName`
   under bare name + qualified alias `<hostNodeName>.camera`, following the
   identical aliasing pattern as `SOMLight`. The `.camera` alias is always
   created unconditionally.
3. **Active-camera selection.** "Which camera is active" is world state.
   Recommended representation: an `activeCamera` name reference in
   `extras.atrium` (Tier A). **Concrete field deferred to the Tier A
   schema session.** `SOMCamera` type work does not block on it.
4. **`SOMCamera` registration is the implementation session's primary
   correctness case** — confirm the `_objectsByName` registration pattern
   established by `SOMLight` applies cleanly.

The implementation brief for `SOMCamera` should be drafted from this
document. It follows the `SOMLight` brief pattern: same packages touched
(som, client, protocol), same registration mechanism, same mutation/event
pattern. Effort should be smaller than `SOMLight` since the pattern is now
established.
...
**Suggested session ordering:** `SOMCamera` first (establishes the Tier B
camera type that Stage's camera-fallback logic needs), then `Stage` as its
own session using the three known consumers.
...
### Mutation listener wiring (as of Session 41)
...
| `space-lights.gltf` | **NEW (Session 38, rebuilt Session 41)** — space.gltf geometry + Sun (directional) + LampGlow (point); both use collision-name pattern |

`space-lights.gltf` is generated by `generate-space-lights.js` (reads
`space.gltf` via `NodeIO`, adds `KHR_lights_punctual` lights, writes result).
...
| **`space-lights.gltf` fixture (geometry + lights)** | **✅ Complete (Session 41)** |
| `SOMCamera` completeness (mutable + registered + qualified alias) | 🔜 Next session |
| `Stage` (Tier C renderer-three tenant) | 🔜 After SOMCamera |
...
### SOMCamera gap (new, Session 38 finding)

`SOMCamera` is not registered in `_objectsByName`. Protocol `set` messages
targeting camera intrinsics will fail with `NODE_NOT_FOUND` until the
`SOMCamera` completeness session lands. The `SOMCamera` brief should scope
this as its primary acceptance criterion.
...
- **`som:add` does not carry extension data.** Lights and cameras cannot
  ...
  `som:add` listener needed for lights.
...
- **`SOMCamera` completeness.** Make all properties mutable + mutation-firing.
  Register under bare name + `<hostNode>.camera` qualified alias. Wire
  `_attachCameraListeners` in `AtriumClient`. Same pattern as `SOMLight`;
  brief should be short. Pre-brief verification not required (same mechanism,
  already confirmed by `SOMLight` arc).
- **Full-system QA pass.** Validates Session 37 deferrals (smoke cases 4 and
  5: late-joiner time-sync, live `loop: false` mid-play). Independent of the
  SOMCamera work.

### High impact (after SOMCamera)

- **`Stage` (Tier C).** Absorbs replicated per-app setup across three
  consumers (`apps/client`, `tools/som-inspector`, `apps/playground`).
  Draft brief from this document's Stage section. Implementation session
  after `SOMCamera` is complete.
...
# start a world server (lit world — recommended for SOMLight/SOMCamera smoke)
cd packages/server
...
- `Project_Atrium_2026-05-14.md` — Sessions 1–37 canonical (superseded by this document)

### Design documents (folded into this handoff)
