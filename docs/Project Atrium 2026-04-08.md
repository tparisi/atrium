...
│   └── som-inspector/                  # SOM Inspector (Sessions 19, 22–24)
│       ├── index.html
│       └── src/
│           ├── app.js
│           ├── TreeView.js
...
## External References (Designed Session 21, Implemented Session 24)
...
- `_worldBaseUrl` is set by `loadWorld(url)`, null'd by `loadWorldFromData`
...
- **External ref late-joiner mutation gap** — externally-loaded nodes
  are not in the server's SOM, so `som-dump` for late joiners won't
  include mutations other clients made to external content. Each late
  joiner gets a fresh copy from the source file. (Session 24)
...
- **External refs via `som-dump` (connect-only) have no base URL** —
  ...
  cannot resolve. The Load-then-Connect flow works. Fix requires the
  server to send the world file's base URL (e.g. in `hello` or
  `som-dump`), which is a protocol change. (Session 24)
...
### Architecture
- **World base URL in `hello`/`som-dump`** — server sends the world
  file's base URL so connect-only clients can resolve external references
  without a prior Load. Protocol change needed. (Session 24 gap)
