# Project Atrium
## 2026-06-22 · As of Session 47

This document is the canonical project handoff. It supersedes
`Project_Atrium_2026-06-20.md` (Sessions 1–46). This document is
self-contained — the next session does not need any predecessor. Prior
handoffs are retained in the repo as historical record.

**SOMCamera / `activeCamera` is not working as expected.** Per user
testing, as of Session 47, the SOMCamera implementation is still not
working as expected. Fixes will be designed after additional testing and
a thorough manual code inspection — see "Backlog → Immediate" (top item)
below before doing anything else with camera navigation.
...
## Test Counts (after Session 46; Session 47 not yet reconciled)
...
Note (Session 47): the camera position/orientation seeding logic in
`Stage.setActiveCamera()`/`_syncCamera()` specifically turned out to be
unit-testable after all, via the same stub pattern already used for the
existing ORBIT/WALK assertions in `stage.test.js` — no DOM/WebGL
required for that part. The broader `activeCamera`/SOM Inspector
integration still requires a live `sceneGroup` as described above.
...
## SOMCamera / `activeCamera` — Current Status

**Per user testing, as of Session 47, the SOMCamera implementation is
still not working as expected.** Fixes will be designed after additional
testing and a thorough manual code inspection — see Backlog → Immediate
(top item). Prior design history and architecture for `activeCamera`
Phase 2 are documented above for reference; the debugging detail from
the Session 47 investigation has been intentionally removed from this
handoff pending that further review.
...
parent-relative conversion), then `renderer.render()`.
...
| `apps/client` — full UI | ✅ Complete (see SOMCamera status note above) |
...
| **Camera position/orientation correct when bound to a `SOMCamera` in WALK/FLY mode** | 🔴 **Not working as expected per user testing (Session 47) — see Backlog → Immediate** |
...
### 🔴 SOMCamera / `activeCamera` (read this first)

- **Per user testing, as of Session 47, the SOMCamera implementation is
  still not working as expected.** Fixes will be designed after
  additional testing and a thorough manual code inspection — see
  Backlog → Immediate (top item).
...
- **SOM Inspector and `apps/client` diverge on default `navMode` (ORBIT vs.
  ...
  legitimate and thorough — but it exercised only ORBIT mode, leaving
  WALK/FLY-mode camera binding unexercised. A feature "working" in the
  tool built to test it doesn't mean it works in the app it's ultimately
  meant to serve, if the two default to different code paths. (See
  SOMCamera status note above — this gap is part of what's now under
  renewed testing and review.)
...
- **Additional SOMCamera testing and code review.** Per user testing, as
  of Session 47, the SOMCamera implementation is still not working as
  expected. No further fix attempts until additional manual testing and a
  thorough manual code inspection are done — see SOMCamera status note
  above. Do not treat this as a quick patch; this is a chat-design-
  conversation item first, same as every other arc.
...
- `Project_Atrium_2026-06-18.md` — Sessions 1–44 canonical (superseded)
...
- `Project_Atrium_2026-06-20.md` — Sessions 1–46 canonical (superseded by
  this document)
