# Atrium — Claude Code Session: README

## Goal

Populate `README.md` in the repo root with the draft content provided below.

Verify every technical detail against the actual codebase and correct anything
that is inaccurate — directory structure, package names, test counts, install
commands, file paths, status table. Fix technical errors silently; do not
change the vision, design principles, or prose tone in any way.
...
2. Walk the actual repo to verify all technical claims:
   - Directory and file structure
   - Package names in `packages/*/package.json`
   - Test counts (run `pnpm test` and count passing tests per package)
   - Getting started commands (verify they actually work)
   - Status table accuracy
3. Write the corrected content to `README.md` in the repo root
4. Do not change any prose in the Why Atrium Exists, Design Principles, or
   How It Works sections — those are final and not for Claude Code to edit
5. Do not add anything not in the draft — no extra sections, no badges,
...
There are 26 passing tests covering protocol validation, session lifecycle,
...
```bash
# clone and install
git clone https://github.com/tonyparisi/atrium.git
cd atrium
pnpm install
...
1. Verify `README.md` renders correctly in VS Code markdown preview
2. Confirm all technical details are accurate
3. Report any corrections made and why
